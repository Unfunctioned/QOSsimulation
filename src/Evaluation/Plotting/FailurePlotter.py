...
class FailurePlotter:
    ...
    def plot(fileName : Path):
        ...
        ax2.plot(data['CASE'], data['AREA_FAILS'] / data['TOTAL_FAILS'])
        ax2.plot(data['CASE'], data['PATH_FAILS'] / data['TOTAL_FAILS'])
        ax2.plot(data['CASE'], data['TRAJECTORY_FAILS'] / data['TOTAL_FAILS'])
        ...
        ax3.plot(data['CASE'], data['CAPACITY_FAILS'] / data['TOTAL_FAILS'])
        ax3.plot(data['CASE'], data['LATENCY_FAILS'] / data['TOTAL_FAILS'])
        ax3.legend(['Capacity Fails', 'Latency Fails'], loc = 'center right')
        ax3.set_title("Absolute Failure Type Distribution")
        ax3.set_xlabel('Simulation Number (#)')
        ax3.set_ylabel("Relative Failure Amount (%)")
